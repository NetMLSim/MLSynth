...
class NICWrapper(Wrapper):
    def __init__(self, model, variance=1):
        self.model = model
    ...
    def insert_slowdown(self, ops: list[ChakraNode], layer: int) -> list[ChakraNode]:
        ...
        for i in range(len(ops) - 1, -1, -1):
            ...
            if op.type != ChakraNodeType.COMP_NODE:
                delay_node = compute(0, 0, parents=[op], name=f"{op.name}_delay")

                ops[i].data_deps = [delay_node.id]
                ops.insert(i, delay_node)
        return ops